...
@inline MeasureBase.testvalue(::Type{T}, d::Distribution) where {T} = testvalue(T, basemeasure(d))


@inline MeasureBase.basemeasure(d::Distributions.Poisson) =
    Counting(MeasureBase.BoundedInts(static(0), static(Inf)))
@inline MeasureBase.basemeasure(d::Distributions.Product{<:Any,<:Distributions.Poisson}) =
