...
The standard normal distribution, scalar (`N == 0`) or as a product over an
array of rank `N`.
"""
const StandardNormal{N} = StandardDist{Normal,N}
...
Base.convert(::Type{Distributions.MvNormal}, d::StandardDist{Normal,1}) =
...
Statistics.median(d::StandardDist{Normal}) = Statistics.mean(d)
...
Statistics.std(d::StandardDist{Normal,0}) = 1
Statistics.std(d::StandardDist{Normal,N}) where {N} = Ones{Int}(size(d)...)
...
Distributions.logpdf(d::StandardDist{Normal,0}, x::U) where {U<:Real} =
    muladd(abs2(x), -U(1) / U(2), -log2π / U(2))
Distributions.pdf(d::StandardDist{Normal,0}, x::U) where {U<:Real} =
...
Random.rand!(rng::AbstractRNG, d::StandardDist{Normal,N}, x::AbstractArray{<:Real,N}) where {N} =
    Random.randn!(rng, x)
