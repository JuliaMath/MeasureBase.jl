...
StandardDist{D}() where {D<:Distribution{Univariate,Continuous}} =
    StandardDist{D,0,Int}(())
StandardDist{D}(dims::Vararg{U,N}) where {D<:Distribution{Univariate,Continuous},N,U<:Integer} =
...
for (A, B) in [
    ...
    @eval begin
        ...
        # instead of wrapping:
        MeasureBase.AbstractMeasure(::StandardDist{$A,0}) = $B()
        MeasureBase.AbstractMeasure(d::StandardDist{$A,N}) where {N} = $B()^size(d)

        Distributions.Distribution(::$B) = StandardDist{$A}()
        Base.convert(::Type{Distribution}, ::$B) = StandardDist{$A}()

        function Distributions.Distribution(m::PowerMeasure{$B})
            StandardDist{$A}(map(dynamic ∘ length, m.axes)...)
        end
...
(::Type{Distributions.Product})(d::StandardDist{D,1}) where {D} =
    Distributions.Product(fill(StandardDist{D}(), length(d)))

Base.convert(::Type{D}, d::StandardDist{D,0}) where {D<:Distribution{Univariate,Continuous}} = D(d)
Base.convert(::Type{Distributions.Product}, d::StandardDist{D,1}) where {D} =
    Distributions.Product(d)
...
Base.eltype(::Type{<:StandardDist}) = Float64
...
@inline Distributions.logpdf(d::StandardDist{D,0}, x::U) where {D,U} =
...
# Explicit N=1/N=2 methods to avoid dispatch ambiguities with Distributions:
...
Distributions.gradlogpdf(d::StandardDist{D,0}, x::Real) where {D} =
...
Random.rand!(rng::AbstractRNG, d::StandardDist{D,0}, x::AbstractArray{<:Real,0}) where {D} =
    (x[] = rand(rng, d); return x)
Random.rand!(rng::AbstractRNG, d::StandardDist{D,N}, x::AbstractArray{<:Real,N}) where {D,N} =
    rand!(rng, StandardDist{D}(), x)


Distributions.truncated(d::StandardDist{D,0}, l::Real, u::Real) where {D} =
    Distributions.truncated(nonstddist(d), l, u)

Distributions.product_distribution(dists::AbstractVector{<:StandardDist{D,0}}) where {D} =
    StandardDist{D}(size(dists)...)
Distributions.product_distribution(dists::AbstractArray{<:StandardDist{D,0}}) where {D} =
    StandardDist{D}(size(dists)...)
