...
The standard uniform distribution, scalar (`N == 0`) or as a product over an
array of rank `N`.
"""
const StandardUniform{N} = StandardDist{Uniform,N}
...
Statistics.median(d::StandardDist{Uniform,0}) = Statistics.mean(d)
...
Statistics.std(d::StandardDist{Uniform,0}) = sqrt(Statistics.var(d))
...
Distributions.logcdf(d::StandardDist{Uniform,0}, x::U) where {U<:Real} =
    log(Distributions.cdf(d, x))

function Distributions.cdf(d::StandardDist{Uniform,0}, x::U) where {U<:Real}
    ifelse(x < zero(U), zero(U), ifelse(x < one(U), x, one(U)))
end

Distributions.logccdf(d::StandardDist{Uniform,0}, x::U) where {U<:Real} =
    log(Distributions.ccdf(d, x))

Distributions.ccdf(d::StandardDist{Uniform,0}, x::U) where {U<:Real} =
    one(x) - Distributions.cdf(d, x)
...
Random.rand!(rng::AbstractRNG, d::StandardDist{Uniform,N}, x::AbstractArray{<:Real,N}) where {N} =
    rand!(rng, x)
