...
@testset "shape contract for Distributions" begin
    ...
    @test @inferred(preferred_stdmeasure(Beta(2, 3))) === StdLogistic
    @test @inferred(preferred_stdmeasure(truncated(Normal(), 0, 1))) === StdLogistic
    ...
    @test @inferred(preferred_stdmeasure(productmeasure((a = Dirac(1.0), b = asmeasure(Beta(2, 3)))))) === StdLogistic
    ...
    @test @inferred(preferred_stdmeasure(product_distribution([Beta(2, 3), Beta(1, 1)]))) === StdLogistic
